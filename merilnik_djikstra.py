...
from djikstra_s_t import djikstra_s_t
...
def oceni_potreben_cas_djikstra_s_t(fun, gen_graf, n, k):
    """ Funkcija oceni potreben čas za izvedbo funkcije `fun` na primerih
    dolžine `n`. Za oceno generira primere primerne dolžine s klicom
    `gen_primerov(n)`, in vzame povprečje časa za `k` primerov. """
    ...
    zac = time.perf_counter()
    for _ in range(k):
        fun(gen_graf(n)[1], 0,random.randint(1,n))
    konc = time.perf_counter()
    return (konc - zac) / k
...
def primerjaj_case_dveh(fun1, fun2, gen_graf, sez_n, k=5):
    ...
    for el in sez_n:
        ...
        casi2.append(oceni_potreben_cas_djikstra_s_t(fun2, gen_graf, el, k))
    ...
    plt.plot(sez_n, casi2, label = 'Dodelan Djikstra') 
...
if __name__ == '__main__':
    print(primerjaj_case_dveh(djikstra, djikstra_s_t, gen_graf, [100,200,300,400,500,600,700,800,900,1000], k=10))
