...
#     [1, 0, 0, 10, 0, 0]
# =============================================================================
def najboljsa_zaloga(predmeti, W):
    '''Vrne seznam "zaloga" dolzine "len(predmeti)", kjer "zaloga[i]" predstavlja koliko zaloge
        potrebuje za predmet "i", tako da bo skupna vrednost posiljke na
        letalu z maksimalno nosilnostjo "W" cim vecja'''
...
def tovor_rezanje(predmeti, W):
    '''Vrne koliko je sedaj najvecja skupna cena tovora, ce je nosilnost letala
        najvec "W". Rezultat vrni zaokrozen na dve decimalni mesti.'''
    ...
    def memo(i, w):
        ...
        return max(
            memo(i-1, w),
            memo(i-1, w-predmeti[i-1][1]) + predmeti[i-1][0],
            memo(i-1, w-predmeti[i-1][1]/2) + predmeti[i-1][0]/2,
            memo(i-1, w-predmeti[i-1][1]/3) + predmeti[i-1][0]/3,
            memo(i-1, w-predmeti[i-1][1]/4) + predmeti[i-1][0]/4
            )
    
    return round(memo(len(predmeti), W), 2)
# =====================================================================@033285=
# 7. podnaloga
...
#     35
# =============================================================================
def tovor_novi_predmet(predmeti, W, novi_predmeti):
    '''Vrne najboljso ceno tovora, ki ga lahko spravimo na letalo z nosilnostjo
        najvec "W" z predmeti iz seznama "predmeti" ter enim dodatnim predmetom
        iz seznama "novi_predmeti".'''
    ...
    def memo(i, w):
        ...
        if i == 0 or w == 0:
            return 0
        if w - predmeti[i-1][1] >= 0:
            return max(memo(i-1, w), memo(i-1, w - premdeti[i-1][1]) + predmeti[i-1][0])
        
        return memo(i-1, w)
    
    M = -1
    n = len(predmeti)
    for c, v in novi_predmeti:
        vrednost = max(memo(n, W), memo(n, W - v) + c)
        if vrednost > M:
            M = vrednost
    return M
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzI1MywidXNlciI6NDAwM30:1pbna2:Taa2jjsty16lvjD6uMXDNWA3Wjsj62WSY2j6WUQgiD4"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzI1NCwidXNlciI6NDAwM30:1pbna2:vxkYRzf299hB5-8yEa4Gum0OtuUzrV8f8qm2ZJ9TlO4"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzI1NSwidXNlciI6NDAwM30:1pbna2:YayiyZqU4mJuMSZhgaxD-87gvBDBZkhFqgwWHsxgYR4"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzI1NiwidXNlciI6NDAwM30:1pbna2:VBA163JZ0WUdlae2IOhPpxugXPsD0u4YgLYkS5gPGqM"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzI1NywidXNlciI6NDAwM30:1pbna2:72sEZWwpnHV8hbVhJEUKIKQ10Z_VCP8m4KjvAUfwW3U"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzI4NCwidXNlciI6NDAwM30:1pbna2:5naFPaLjzcI_gFEu72BnbeAPBXW3miPV6CQj9iXvYcE"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzI4NSwidXNlciI6NDAwM30:1pbna2:s19IRt_Kd2hs01CeHvAgjaIB9vtt5kgccFTFjVa0o0E"
        try:
    ...
    try:
        ...
        token = "Token 15df823ba6f256d9724def9ae163520921514131"
