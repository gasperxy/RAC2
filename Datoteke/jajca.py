...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzA4NCwidXNlciI6MzkzNX0:1ph44G:PA2bIz7P6cn33fgS67L0MF5Y25AS2Ei1RtSa8m6F4Z0"
        try:
            Check.equal('jajce_rec(2, 36)', 8) and \
            Check.equal('jajce_rec(3, 112)', 9) and \
            Check.equal('jajce_rec(3, 300)', 13) and \
            Check.equal('jajce_rec(2, 321)', 25)
        except:
            Check.error(
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzA4NSwidXNlciI6MzkzNX0:1ph44G:Zk17FVWh_m6IJL1UyZbbxrQ_zyWhzQGi8tk_QWoBcHM"
        try:
            Check.equal('jajce_iter(2, 1111)', 47) and \
            Check.equal('jajce_iter(35, 1054)', 11) and \
            Check.equal('jajce_iter(5, 1678)', 13) and \
            Check.equal('jajce_iter(21, 1555)', 11)
        except:
            Check.error(
                "Testi sprožijo izjemo\n  {0}",
                "\n  ".join(traceback.format_exc().split("\n"))[:-2],
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzA4NiwidXNlciI6MzkzNX0:1ph44G:jqtOFx4GG-F7WZcB3BRNmNjwWRe2FOHxyagTAF6-UUc"
        try:
            Check.equal('nadstropja(5, 4)', 30) and \
            Check.equal('nadstropja(15, 4)', 1940) and \
            Check.equal('nadstropja(11, 9)', 2035) and \
            Check.equal('nadstropja(12, 1)', 12)
        except:
            Check.error(
                "Testi sprožijo izjemo\n  {0}",
                "\n  ".join(traceback.format_exc().split("\n"))[:-2],
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjozMzA4NywidXNlciI6MzkzNX0:1ph44G:ow_8M6gC92H5GQRo21X_LrRzW7tZ_KPrTluthtmqohg"
        try:
            Check.equal('jajca_hitro(2, 1111)', 47) and \
            Check.equal('jajca_hitro(35, 9567)', 14) and \
            Check.equal('jajca_hitro(21, 254672)', 18) and \
            Check.equal('jajca_hitro(10234, 254672)', 18)
        except:
            Check.error(
                "Testi sprožijo izjemo\n  {0}",
                "\n  ".join(traceback.format_exc().split("\n"))[:-2],
    ...
    try:
        ...
        token = "Token 44eadea4e0e5d4cdc9316392da42a5fbb9fcc070"
